# References and further reading

Links below open the publication record in the
[NASA Astrophysics Data System (ADS)](https://ui.adsabs.harvard.edu/). This is
not an exhaustive bibliography; the source docstrings identify the relation
used by each function.

## Relations and priors implemented by AsteroScale

- [Kjeldsen & Bedding (1995), *Amplitudes of stellar oscillations: the implications for asteroseismology*](https://ui.adsabs.harvard.edu/abs/1995A%26A...293...87K/abstract)
- [Huber et al. (2011), *Testing Scaling Relations for Solar-like Oscillations from the Main Sequence to Red Giants Using Kepler Data*](https://ui.adsabs.harvard.edu/abs/2011ApJ...743..143H/abstract)
- [Ballot et al. (2011), *Visibilities and bolometric corrections for stellar oscillation modes observed by Kepler*](https://ui.adsabs.harvard.edu/abs/2011A%26A...531A.124B/abstract)
- [Lund, Ballot & Chaplin (2026), *Bolometric corrections of stellar oscillation mode amplitudes as observed by the PLATO mission. I. Planck-spectrum estimates*](https://arxiv.org/abs/2603.12750)
- [Mosser et al. (2012), *Characterization of the power excess of solar-like oscillations in red giants with Kepler*](https://ui.adsabs.harvard.edu/abs/2012A%26A...537A..30M/abstract)
- [Kallinger et al. (2014), *The connection between stellar granulation and oscillation as seen by the Kepler mission*](https://ui.adsabs.harvard.edu/abs/2014A%26A...570A..41K/abstract)
- [Bailer-Jones (2015), *Estimating Distances from Parallaxes*](https://ui.adsabs.harvard.edu/abs/2015PASP..127..994B/abstract)
- [Campante et al. (2016), *The Asteroseismic Potential of TESS*](https://ui.adsabs.harvard.edu/abs/2016ApJ...830..138C/abstract)
- [Guggenberger et al. (2016), *Significantly improving stellar mass and radius estimates: a new reference function for the $\Delta\nu$ scaling relation*](https://ui.adsabs.harvard.edu/abs/2016MNRAS.460.4277G/abstract)
- [Viani et al. (2017), *Changing the $\nu_{\max}$ Scaling Relation: The Need for a Mean Molecular Weight Term*](https://ui.adsabs.harvard.edu/abs/2017ApJ...843...11V/abstract)
- [Ball et al. (2018), *A Synthetic Sample of Short-cadence Solar-like Oscillators for TESS*](https://ui.adsabs.harvard.edu/abs/2018ApJS..239...34B/abstract)

## Gaia synthetic photometry

AsteroScale's Gaia DR3 bolometric corrections and extinction conversions are
derived from the MARCS-based tables accompanying:

- [Casagrande & VandenBerg (2014), *Synthetic Stellar Photometry - I. General considerations and new transformations for broad-band systems*](https://ui.adsabs.harvard.edu/abs/2014MNRAS.444..392C/abstract)
- [Casagrande & VandenBerg (2018a), *Synthetic stellar photometry - II. Testing the bolometric flux scale and tables of bolometric corrections for the Hipparcos/Tycho, Pan-STARRS1, SkyMapper, and JWST systems*](https://ui.adsabs.harvard.edu/abs/2018MNRAS.475.5023C/abstract)
- [Casagrande & VandenBerg (2018b), *On the use of Gaia magnitudes and new tables of bolometric corrections*](https://ui.adsabs.harvard.edu/abs/2018MNRAS.479L.102C/abstract)

## Sources used in the real-star tutorial

- [Kjeldsen et al. (2005), oscillations in $\alpha$ Centauri A](https://ui.adsabs.harvard.edu/abs/2005ApJ...635.1281K/abstract)
- [Kervella et al. (2017), interferometric radii of $\alpha$ Centauri A and B](https://ui.adsabs.harvard.edu/abs/2017A%26A...597A.137K/abstract)
- [Arentoft et al. (2019), asteroseismology of $\epsilon$ Tauri](https://ui.adsabs.harvard.edu/abs/2019A%26A...622A.190A/abstract)
- [Campante et al. (2024), detection of solar-like oscillations in $\epsilon$ Indi A](https://ui.adsabs.harvard.edu/abs/2024A%26A...683L..16C/abstract)
- [Lundkvist et al. (2024), low-amplitude oscillations in $\epsilon$ Indi A](https://ui.adsabs.harvard.edu/abs/2024ApJ...964..110L/abstract)

## Introductory reviews

These reviews go well beyond AsteroScale, but are useful next steps:

- [Chaplin & Miglio (2013), *Asteroseismology of Solar-Type and Red-Giant Stars*](https://ui.adsabs.harvard.edu/abs/2013ARA%26A..51..353C/abstract) — broad introduction and astronomical applications.
- [Hekker & Christensen-Dalsgaard (2017), *Giant star seismology*](https://ui.adsabs.harvard.edu/abs/2017A%26ARv..25....1H/abstract) — evolved stars and mixed modes.
- [García & Ballot (2019), *Asteroseismology of solar-type stars*](https://ui.adsabs.harvard.edu/abs/2019LRSP...16....4G/abstract) — observations, power spectra, and stellar interiors.
- [Aerts (2021), *Probing the interior physics of stars through asteroseismology*](https://ui.adsabs.harvard.edu/abs/2021RvMP...93a5001A/abstract) — a wider review covering several classes of pulsating stars.
