...
- `dnu`: the average spacing between consecutive radial orders;
- `A_env`: the maximum radial-mode RMS amplitude in the selected band;
- `A_gran`: the characteristic granulation RMS amplitude.

See {doc}`concepts/asteroseismology-for-exoplanets` before interpreting these as a
detectability prediction or time-domain noise estimate.

## Include measurement uncertainties

A plain number is treated as exact. A `(value, uncertainty)` pair represents
...
Continue with the {doc}`tutorials/index` or consult the
{doc}`reference/quantities` table for names and units.
